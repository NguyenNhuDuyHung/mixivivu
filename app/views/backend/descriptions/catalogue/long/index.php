<div class="page">
    <div class="page-container">
        ...
        <div class="flex page-action">
            <a href="<?= _WEB_ROOT ?>/backend/long-desc/catalogue/create">
                <button type="button" class="btn btn-normal btn-primary">Tạo nhóm long-desc</button>
            </a>

            <form class="page-action-search search-box-input-group" method="get"
                action="<?= _WEB_ROOT ?>/backend/long-desc/catalogue/search" id="LongDescCatalogueSearchForm">
                <div class="search-box-search-input search-box-search-input">
                    <label for="" class="input-group">
                        ...
                        <input class="p-md" type="text" name="keyword" placeholder="Tìm kiếm nhóm long-desc"
                ...
                <div class="page-content-item-body">
                    <table class="custom-table">
                        ...
                        <tbody>
                            ...
                            ?>
                            <?php foreach ($data['long_desc_types'] as $index => $longDesc):
                                $count++ ?>
                                <tr>
                                    <td><?= $startNumber + $index ?></td>
                                    <td><?= $longDesc['type'] ?></td>
                                    <td><?= $longDesc['ld_count'] ?? 0 ?></td>
                                    <td>
                                        <div style="display: flex; align-items: center; gap: 0 16px;">
                                            <a href="<?= _WEB_ROOT ?>/backend/long-desc/catalogue/update/<?= $longDesc['id'] ?>">
                                            ...
                                            <a href="<?= _WEB_ROOT ?>/backend/long-desc/catalogue/delete/<?= $longDesc['id'] ?>">
                                                <button class="btn btn-normal btn-outline btn-iconOnly">
        ...
        <div class="flex justify-between align-center Pagination-pagination">
            ...
            <ul class="Pagination-pagination-container">
                <a href="<?= _WEB_ROOT ?>/backend/long-desc/catalogue<?= isset($_GET['keyword']) ? '/search?keyword=' . $_GET['keyword'] . '&page=' . $data['currentPage'] - 1 : '/page/' . $data['currentPage'] - 1 ?>"
                ...
                <?php for ($i = 1; $i <= $data['numberPage']; $i++): ?>
                    <a href="<?= _WEB_ROOT ?>/backend/long-desc/catalogue<?= isset($_GET['keyword']) ? '/search?keyword=' . $_GET['keyword'] . '&page=' . $i : '/page/' . $i ?>"
