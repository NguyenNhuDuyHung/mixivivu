<div class="page">
    <div class="page-container">
        ...
        <div class="flex page-action">
            <a href="<?= _WEB_ROOT ?>/backend/long-desc/create">
                <button type="button" class="btn btn-normal btn-primary">Tạo mô tả</button>
            </a>

            <form class="page-action-search search-box-input-group" method="get" action="<?= _WEB_ROOT ?>/backend/long-desc/search"
                id="LongDescSearchForm">
                ...
                <div class="page-content-item-body">
                    <table class="custom-table">
                        ...
                        <tbody>
                            ...
                            <?php foreach ($data['long_descs'] as $index => $longDesc):
                                $count++ ?>
                                <tr>
                                    <td><?= $startNumber + $index ?></td>
                                    <td><?= $longDesc['title'] ?></td>
                                    <td><?= $longDesc['countDesc'] ?></td>
                                    <td>
                                        <div style="display: flex; align-items: center; gap: 0 16px;">
                                            <a href="<?= _WEB_ROOT ?>/backend/long-desc/update/<?= $longDesc['id'] ?>">
                                            ...
                                            <a href="<?= _WEB_ROOT ?>/backend/long-desc/delete/<?= $longDesc['id'] ?>">
                                                <button class="btn btn-normal btn-outline btn-iconOnly">
        ...
        <div class="flex justify-between align-center Pagination-pagination">
            ...
            <ul class="Pagination-pagination-container">
                <a href="<?= _WEB_ROOT ?>/backend/long-desc<?= isset($_GET['keyword']) ? '/search?keyword=' . $_GET['keyword'] . '&page=' . $data['currentPage'] - 1 : '/page/' . $data['currentPage'] - 1;
                                                            ?>"
                ...
                <?php for ($i = 1; $i <= $data['numberPage']; $i++): ?>
                    <a href="<?= _WEB_ROOT ?>/backend/long-desc<?= isset($_GET['keyword']) ? '/search?keyword=' . $_GET['keyword'] . '&page=' . $i : '/page/' . $i ?>"
