<div class="page">
    <div class="page-container">
        ...
        <div class="flex page-action">
            <a href="<?= _WEB_ROOT ?>/backend/ship/catalogue/create">
                <button type="button" class="btn btn-normal btn-primary">Thêm nhóm du thuyền</button>
            </a>

            <form class="page-action-search search-box-input-group" method="get"
                action="<?= _WEB_ROOT ?>/backend/ship/catalogue/search" id="ShipCatalogueSearchForm">
                <div class="search-box-search-input search-box-search-input">
                    <label for="" class="input-group">
                        ...
                        <input class="p-md" type="text" name="keyword" placeholder="Tìm kiếm nhóm du thuyền"
                ...
                <div class="page-content-item-body">
                    <table class="custom-table">
                        ...
                        <tbody>
                            ...
                            ?>
                            <?php foreach ($data['shipCatalogues'] as $index => $shipCatalogue):
                                $count++ ?>
                                <tr>
                                    ...
                                    <td>
                                        <div style="display: flex; align-items: center; gap: 0 16px;">
                                            ...
                                            <a href="<?= _WEB_ROOT ?>/backend/ship/catalogue/delete/<?= $shipCatalogue['id'] ?>">
                                                <button class="btn btn-normal btn-outline btn-iconOnly">
        ...
        <div class="flex justify-between align-center Pagination-pagination">
            ...
            <ul class="Pagination-pagination-container">
                <a href="<?= _WEB_ROOT ?>/backend/ship/catalogue<?= isset($_GET['keyword']) ? '/search?keyword=' . $_GET['keyword'] . '&page=' . $data['currentPage'] - 1 : '/page/' . $data['currentPage'] - 1;
                                                                ?>"
                ...
                <?php for ($i = 1; $i <= $data['numberPage']; $i++): ?>
                    <a href="<?= _WEB_ROOT ?>/backend/ship/catalogue<?= isset($_GET['keyword']) ? '/search?keyword=' . $_GET['keyword'] . '&page=' . $i : '/page/' . $i ?>"
