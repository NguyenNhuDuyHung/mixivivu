    ...
    <div class="page-container">
        ...
        <div class="flex page-action">
            <form class="page-action-search search-box-input-group" method="get"
                action="<?= _WEB_ROOT ?>/backend/order/search" id="OrdersearchForm">
                <div class="search-box-search-input search-box-search-input">
                    <label for="" class="input-group">
                        ...
                        <input class="p-md" type="text" name="keyword" placeholder="Tìm kiếm theo email khách hàng"
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div style="display: flex; align-items: center; gap: 0 16px;">
                                            <a href="<?= _WEB_ROOT ?>/backend/order/update/<?= $order['booking_id'] ?>">
        ...
        <div class="flex justify-between align-center Pagination-pagination">
            ...
            <ul class="Pagination-pagination-container">
                <a href="<?= _WEB_ROOT ?>/backend/order<?= isset($_GET['keyword']) ? '/search?keyword=' . $_GET['keyword'] . '&page=' . $data['currentPage'] - 1 : '/page/' . $data['currentPage'] - 1;
                                                        ?>"
                ...
                <?php for ($i = 1; $i <= $data['numberPage']; $i++): ?>
                    <a href="<?= _WEB_ROOT ?>/backend/order<?= isset($_GET['keyword']) ? '/search?keyword=' . $_GET['keyword'] . '&page=' . $i : '/page/' . $i ?>"
