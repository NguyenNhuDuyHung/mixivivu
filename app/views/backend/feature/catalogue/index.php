<div class="page">
    <div class="page-container">
        ...
        <div class="flex page-action">
            <a href="<?= _WEB_ROOT ?>/backend/feature/catalogue/create">
                <button type="button" class="btn btn-normal btn-primary">Tạo loại đặc trưng</button>
            </a>

            <form class="page-action-search search-box-input-group" method="get" action="<?= _WEB_ROOT ?>/backend/feature/catalogue/search"
                id="FeatureCatalogueSearchForm">
                ...
                <div class="page-content-item-body">
                    <table class="custom-table">
                        ...
                        <tbody>
                            ...
                            ?>
                            <?php foreach ($data['featureCatalogues'] as $index => $feature):
                                $count++ ?>
                                <tr>
                                    ...
                                    <td>
                                        <div style="display: flex; align-items: center; gap: 0 16px;">
                                            <a href="<?= _WEB_ROOT ?>/backend/feature/catalogue/update/<?= $feature['id'] ?>">
                                            ...
                                            <a href="<?= _WEB_ROOT ?>/backend/feature/catalogue/delete/<?= $feature['id'] ?>">
                                                <button class="btn btn-normal btn-outline btn-iconOnly">
        ...
        <div class="flex justify-between align-center Pagination-pagination">
            ...
            <ul class="Pagination-pagination-container">
                <a href="<?= _WEB_ROOT ?>/backend/feature/catalogue<?= isset($_GET['keyword']) ? '/search?keyword=' . $_GET['keyword'] . '&page=' . $data['currentPage'] - 1 : '/page/' . $data['currentPage'] - 1;
                                                            ?>"
                ...
                <?php for ($i = 1; $i <= $data['numberPage']; $i++): ?>
                    <a href="<?= _WEB_ROOT ?>/backend/feature/catalogue<?= isset($_GET['keyword']) ? '/search?keyword=' . $_GET['keyword'] . '&page=' . $i : '/page/' . $i ?>"
