<div class="action-page">
    <form method="post" action="" id="HotelUpdateForm" enctype="multipart/form-data">
        <div class="modal">
                    ...
                    <div class="">
                        <label for="title" class="input-group">
                            <input id="title" class="p-md" placeholder="Nhập tiêu đề" name="title"
                                value="<?php echo $this->oldInfo('title', $data) ?? $data['hotel']['title'] ?>" autocomplete="off">
                    ...
                    <div class="">
                        <label for="address" class="input-group">
                            <input id="address" class="p-md" placeholder="Nhập địa chỉ" name="address"
                                value="<?php echo $this->oldInfo('address', $data) ?? $data['hotel']['address'] ?>" autocomplete="off">
                    ...
                    <div class="select-input">
                        <label for="city_id" class="input-group">
                            <input id="city_id" class="p-md" placeholder="Chọn thành phố" name="city_id"
                                value="<?php echo $this->oldInfo('city_id', $data) ?? $data['hotel']['city_id'] ?>" autocomplete="off">
                    ...
                    <div class="">
                        <label for="map_link" class="input-group">
                            <input id="map_link" class="p-md" placeholder="Nhập link bản đồ google map" name="map_link"
                                value="<?php echo $this->oldInfo('map_link', $data) ?? $data['hotel']['map_link'] ?>" autocomplete="off"
                                type="text">
                    ...
                    <div class="">
                        <label for="map_iframe_link" class="input-group">
                            <input id="map_iframe_link" class="p-md" placeholder="Nhập link nhúng bản đồ google map cho thẻ iframe" name="map_iframe_link"
                                value="<?php echo $this->oldInfo('map_iframe_link', $data)  ?? $data['hotel']['map_iframe_link'] ?>" autocomplete="off">
                    ...
                    <div class="">
                        <label for="default_price" class="input-group">
                            <input id="default_price" class="p-md" placeholder="Nhập giá" name="default_price"
                                value="<?php echo $this->oldInfo('default_price', $data) ?? $data['hotel']['default_price'] ?>" autocomplete="off">
                    ...
                    <div class="">
                        <label for="slug" class="input-group">
                            <input id="slug" class="p-md" placeholder="Slug" name="slug"
                                value="<?php echo $this->oldInfo('slug', $data) ?? $data['hotel']['slug'] ?>" autocomplete="off"
                                type="text">
                    ...
                    <div class="select-input">
                        <label for="active" class="input-group">
                            <input id="active" class="p-md" placeholder="Chọn kích hoạt" name="active"
                                value="<?php echo $this->oldInfo('active', $data) ?? $data['hotel']['active'] == 1 ? 'Kích hoạt' : 'Không kích hoạt' ?>" autocomplete="off">
                    ...
                    <div class="">
                        <label for="admin" class="input-group">
                            <input id="admin" class="p-md" placeholder="Nhập tên công ty điều hành" name="admin"
                                value="<?php echo $this->oldInfo('admin', $data) ?? $data['hotel']['admin'] ?>" autocomplete="off">
                    ...
                    <div class="">
                        <label for="upload-thumbnail" class="flex align-center">
                            <div class="flex align-center gap-16">
                                <input type="file" id="upload-thumbnail" name="thumbnail[]" value="<?php echo $this->oldInfo('thumbnail', $data) ?? $data['hotel']['thumbnail'] ?>">
                                <label for="upload-thumbnail" class="upload-thumb-btn btn btn-primary btn-normal">
                                    Choose file
                                </label>
                                <div class="preview-thumb">
                                    <?php
                                    if (!empty($data['hotel']['thumbnail'])) {
                                        echo '<img class="preview-upload-image" src="'  . $data['hotel']['thumbnail'] . '" alt="">';
                                    } else {
                        ...
                        <label for="upload-image" class="flex align-center">
                            <div class="flex align-center gap-16">
                                ...
                                <div class="preview-image">
                                    <?php
                                    if (!empty($data['hotel']['images'])) {
                                        foreach ($data['hotel']['images'] as $image) {
                ...
                <div class="select-feature">
                    <?php foreach ($data['features'] as $features): ?>
                        <label for="<?= $features['id'] ?>" class="Checkbox-container">
                            <!-- Kiểm tra nếu feature_id có trong mảng hotelFeatures -->
                            <input name="hotelFeatures[]" type="checkbox" id="<?= $features['id'] ?>" value="<?= $features['id'] ?>"
                                <?php echo in_array($features['id'], $data['hotelFeatures']) ? 'checked' : ''; ?>>
