<div class="page">
    <div class="page-container">
        ...
        <div class="flex page-action">
            <a href="<?= _WEB_ROOT ?>/backend/room/create">
                <button type="button" class="btn btn-normal btn-primary">Tạo phòng</button>
            </a>

            <form class="page-action-search search-box-input-group" method="get" action="<?= _WEB_ROOT ?>/backend/room/search"
                id="ShortDescSearchForm">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div style="display: flex; align-items: center; gap: 0 16px;">
                                            <a href="<?= _WEB_ROOT ?>/backend/room/update/<?= $room['product_id'] ?>">
                                            ...
                                            <a href="<?= _WEB_ROOT ?>/backend/room/delete/<?= $room['product_id'] ?>">
                                                <button class="btn btn-normal btn-outline btn-iconOnly">
        ...
        <div class="flex justify-between align-center Pagination-pagination">
            ...
            <ul class="Pagination-pagination-container">
                <a href="<?= _WEB_ROOT ?>/backend/room<?= isset($_GET['keyword']) ? '/search?keyword=' . $_GET['keyword'] . '&page=' . $data['currentPage'] - 1 : '/page/' . $data['currentPage'] - 1;
                                                            ?>"
                ...
                <?php for ($i = 1; $i <= $data['numberPage']; $i++): ?>
                    <a href="<?= _WEB_ROOT ?>/backend/room<?= isset($_GET['keyword']) ? '/search?keyword=' . $_GET['keyword'] . '&page=' . $i : '/page/' . $i ?>"
